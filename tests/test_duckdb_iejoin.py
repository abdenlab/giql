...
_EQUALITY_RESIDUAL_SHAPES = {
    ...
    "not_neq": "NOT (a.strand <> b.strand)",
    "in_col": "a.strand IN (b.strand)",
    ...
    "under_or": "(a.strand = b.strand OR a.score > 5)",
    "mixed_operand": "a.start + b.start = 100",
    "collated": "a.strand COLLATE NOCASE = b.strand",
    "is_not_distinct": "a.strand IS NOT DISTINCT FROM b.strand",
}

#: The shapes above that the strand rewrite claims; every other shape is
#: emitted as written.
_REWRITTEN_SHAPES = (
    "strand_eq",
    "reversed",
...
#: Shapes the naive plan cannot run under every join kind, or whose rows are
#: not deterministic, so they are pinned on the emitted SQL only.
_STRUCTURE_ONLY_SHAPES = {
    "any_eq": "a.score = ANY(list_value(b.score, b.score + 4))",
    "volatile_random": "a.score = b.score + CAST(random() * 3 AS INTEGER)",
    "volatile_nextval": "a.score = b.score + nextval('s')",
}

#: The shapes emitted as written, drawn from both tables.
_KEPT_SHAPES = tuple(
    k for k in _EQUALITY_RESIDUAL_SHAPES if k not in _REWRITTEN_SHAPES
) + tuple(_STRUCTURE_ONLY_SHAPES)


def _residual(kind: str) -> str:
    """Return the residual SQL for a shape id from either shape table."""
    return {**_EQUALITY_RESIDUAL_SHAPES, **_STRUCTURE_ONLY_SHAPES}[kind]
...
def _explain_dynamic_sql(conn, sql: str, half: int = 0) -> str:
    """Return the DuckDB EXPLAIN text of the dynamic SQL for *sql*.
...
def _plan_json(conn, sql: str, half: int = 0) -> dict:
    """Return the DuckDB structured EXPLAIN of the dynamic SQL for *sql*.

    The JSON form carries each join's conditions as an ordered list under
    ``extra_info``, which the text form only draws; the text form stays for
    the operator-presence assertions that read best as substrings.
    """
    plan = conn.execute(
...
def _join_predicate(conn, sql: str) -> exp.Expression:
    ...
    Reads the ``JOIN ... ON`` predicate, or the ``EXISTS`` subquery's
    ``WHERE`` for the SEMI / ANTI form, so a test can assert on the conjunct
    order and shape without matching rendered SQL text.
    """
...
def stranded_peaks_genes(conn):
    """Build peaks / genes whose strand column exercises every equality outcome.
...
class TestTranspileDuckDBIEJoinSQLStructure:
    ...
    def test_transpile_should_rewrite_strand_equality_as_inequality_pair_after_overlap_predicates(  # noqa: E501
        ...
        Given:
            Peaks / genes and a JOIN / SEMI JOIN INTERSECTS query whose ON
            carries ``a.strand = b.strand``.
        When:
    ...
    @pytest.mark.parametrize("join_kind", ["JOIN", "SEMI JOIN"])
    def test_transpile_should_rewrite_strand_equality_when_partition_exceeds_ceiling(
        self, conn, join_kind
    ):
        """Test that the unpartitioned fallback carries the same strand pair.

        Given:
            Stranded peaks / genes over more shared chromosomes than
        ...
        Then:
            The join predicate should carry the chromosome equality, the two
            positional overlap predicates, and the ``>=`` / ``<=`` strand pair,
            with no strand equality left.
        """
        # Arrange
        _make_planner_scale_tables(
            conn, strand=True, chroms=MAX_PER_CHROM_PARTITIONS + 1
        )
        sql = transpile(
            f"SELECT a.start FROM peaks a {join_kind} genes b "
            "ON a.interval INTERSECTS b.interval AND a.strand = b.strand",
        ...
        assert [type(c) for c in conjuncts] == [exp.EQ, exp.LT, exp.GT, exp.GTE, exp.LTE]
        assert conjuncts[3:] == [
            parse_one("a.strand >= b.strand", read="duckdb"),
            parse_one("a.strand <= b.strand", read="duckdb"),
        ]

    @pytest.mark.parametrize("residual_kind", _KEPT_SHAPES)
    def test_transpile_should_keep_equality_residual_when_not_a_strand_pair(
        self, peaks_genes, residual_kind
    ):
        """Test that an equality other than a bare strand pair is emitted as written.

        Given:
            A JOIN INTERSECTS query whose ON residual is one of the shapes the
            rewrite leaves alone: an equality on a non-strand column, over an
            expression, one-sided, under ``NOT`` or ``OR``, mixing both sides
            in one operand, collated, NULL-safe, quantified with ``ANY``,
            carrying a volatile operand, or a strand pair beside another
            cross-side equality.
        When:
            The query is transpiled with ``dialect='duckdb'`` and its
            per-chromosome dynamic SQL is read back and parsed.
        ...
        """
        # Arrange
        residual = _residual(residual_kind)
        sql = transpile(
        ...
        # Assert
        expected = parse_one(residual, read="duckdb")
        conjuncts = (
            list(expected.flatten()) if isinstance(expected, exp.And) else [expected]
        )
        assert [c.unnest() for c in predicate.flatten()][2:] == [
            c.unnest() for c in conjuncts
        ]
        assert predicate.find(exp.GTE, exp.LTE) is None

    @pytest.mark.parametrize("residual_kind", _REWRITTEN_SHAPES)
    def test_transpile_should_rewrite_only_the_strand_pair_when_residual_is_compound(
        ...
        Given:
            ...
            pair -- as ``=``, ``NOT (<>)``, or a single-element ``IN``, in
            either operand order, parenthesized, or beside an inequality.
        When:
            The query is transpiled with ``dialect='duckdb'`` and its
            per-chromosome dynamic SQL is read back and parsed.
        Then:
        ...
        sql = transpile(
            "SELECT a.start FROM peaks a JOIN genes b "
            f"ON a.interval INTERSECTS b.interval AND {_residual(residual_kind)}",
            tables=["peaks", "genes"],
            dialect="duckdb",
        )
        ...
        assert predicate.find(exp.EQ, exp.NEQ, exp.In) is None
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                [Table("peaks", strand_col="str"), Table("genes", strand_col="strand")],
                "a.str = b.strand",
                True,
            ),
            (
                [Table("peaks", strand_col="Strand"), "genes"],
                "a.STRAND = b.strand",
                True,
            ),
            (["peaks"], "a.strand = b.strand", True),
            (
                [Table("peaks", strand_col="str"), Table("genes", strand_col="str")],
                "a.strand = b.strand",
                False,
    ...
    def test_transpile_should_gate_the_rewrite_on_the_registered_strand_column(
        ...
        Given:
            ...
            configs whose ``strand_col`` is a custom name, differs per side,
            differs in case from the column as written, is left to the
            default for an unregistered table, or is ``None``.
        When:
            The query is transpiled with ``dialect='duckdb'`` and its
            per-chromosome dynamic SQL is read back and parsed.
        Then:
            The equality should be rewritten only when both operands name the
            strand column registered, or defaulted, for their side.
        """
        # Arrange
        sql = transpile(
            "SELECT a.start FROM peaks a JOIN genes b "
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            "SELECT a.chrom, a.start, COUNT(b.chrom) AS n FROM peaks a "
            "LEFT JOIN genes b ON a.interval INTERSECTS b.interval "
            "AND a.strand = b.strand GROUP BY a.chrom, a.start",
        ],
        ids=["left_join", "count_overlaps"],
    )
    def test_transpile_should_decline_when_outer_join_carries_strand_equality(
        self, query
    ):
        """Test that a strand equality on an outer join declines the dialect.

        Given:
            A LEFT JOIN INTERSECTS query, plain or in the count_overlaps
            shape, whose ON carries ``a.strand = b.strand``.
        When:
            ``transpile`` is called with ``dialect='duckdb'``.
        Then:
            It should emit the naive plan with no dynamic-SQL scaffolding,
            because the outer-join decomposition admits no ON residual.
        """
        # Act
        sql = transpile(query, tables=["peaks", "genes"], dialect="duckdb")
...
class TestTranspileDuckDBIEJoinExecution:
    ...
    @pytest.mark.parametrize("residual_kind", ["strand_eq", "not_neq", "in_col"])
    def test_inner_join_plan_should_reach_iejoin_when_residual_is_strand_equality(
        self, conn, residual_kind
    ):
        ...
        Given:
            ...
            INTERSECTS query whose ON carries a strand equality spelled as
            ``=``, ``NOT (<>)``, or a single-element ``IN``.
        When:
            The ``dialect='duckdb'`` per-chromosome dynamic SQL is planned with
            ``EXPLAIN``.
        Then:
            It should plan through ``IE_JOIN`` with no ``HASH_JOIN``, sorting on
            the two positional overlap predicates and filtering on the strand
            pair.
        """
        # Arrange
    ...
    def test_inner_join_plan_should_keep_hash_join_when_another_equality_survives(
        self, conn
    ):
        """Test that the strand equality stays a hash key beside another equality.

        Given:
            20k stranded rows per side over four chromosomes and an INNER JOIN
            INTERSECTS query whose ON carries ``a.strand = b.strand AND
            a.start = b.start``.
        When:
            The ``dialect='duckdb'`` per-chromosome dynamic SQL is planned with
            ``EXPLAIN``.
        Then:
            It should plan a ``HASH_JOIN`` keyed on both equalities, since the
            start equality keeps the join off ``IE_JOIN`` regardless and the
            rewrite stands down.
        """
        # Arrange
        _make_planner_scale_tables(conn, strand=True)
        sql = transpile(
            "SELECT a.start FROM peaks a JOIN genes b "
            "ON a.interval INTERSECTS b.interval "
            "AND a.strand = b.strand AND a.start = b.start",
            tables=["peaks", "genes"],
            dialect="duckdb",
        )
        ...
        # Assert
        conditions = _find_operator(plan, "HASH_JOIN")["extra_info"]["Conditions"]
        assert "start = start" in conditions
        assert "strand = strand" in conditions
        assert "strand >= strand" not in conditions

    def test_unpartitioned_plan_should_filter_on_strand_when_partition_exceeds_ceiling(
        self, conn
    ):
        """Test that the unpartitioned fallback carries the strand pair as a filter.

        Given:
            20k stranded rows per side over more shared chromosomes than
        ...
        Then:
            It should plan a ``HASH_JOIN`` keyed on the chromosome with the
            ``>=`` / ``<=`` strand pair among its conditions and no strand
            equality.
        """
        # Arrange
        _make_planner_scale_tables(
        ...
        assert {"strand >= strand", "strand <= strand"} <= set(conditions)
        assert "strand = strand" not in conditions
    ...
    @pytest.mark.parametrize("residual_kind", list(_EQUALITY_RESIDUAL_SHAPES))
    def test_query_should_match_naive_plan_for_equality_residual_shapes(
        self, stranded_peaks_genes, residual_kind, join_kind
    ):
    ...
    def test_query_should_match_naive_plan_when_equality_operands_differ_in_type(
        ...
        """Test that a kept cross-type equality on a non-strand column binds.
    ...
    @pytest.mark.parametrize(
        "chroms", [4, MAX_PER_CHROM_PARTITIONS + 1], ids=["partitioned", "unpartitioned"]
    )
    def test_query_should_raise_binder_error_when_strand_columns_differ_in_type(
        self, conn, chroms
    ):
        """Test that unlike strand types raise the binder error on either branch.

        Given:
            Peaks whose ``strand`` is VARCHAR and genes whose ``strand`` is
            INTEGER, spread over fewer or more shared chromosomes than
            ``MAX_PER_CHROM_PARTITIONS``, and an INNER JOIN INTERSECTS query
            whose ON carries ``a.strand = b.strand``.
        When:
            The query is transpiled with ``dialect='duckdb'`` and executed.
        Then:
            It should raise DuckDB's binder error whichever branch the session
            variable selects, since the rewritten ``>=`` / ``<=`` pair has no
            implicit cast between the two types and is emitted in both.
        """
        # Arrange
        for table, strand in (("peaks", "'1'"), ("genes", "1")):
            conn.execute(f"""
                CREATE TABLE {table} AS
                SELECT 'chr' || (i % {chroms}) AS chrom,
                       (i * 37)::INTEGER AS "start",
                       (i * 37 + 200)::INTEGER AS "end",
                       {strand} AS strand
                FROM range({chroms * 4}) t(i)
            """)
        sql = transpile(
            "SELECT a.chrom, a.start FROM peaks a JOIN genes b "
            "ON a.interval INTERSECTS b.interval AND a.strand = b.strand",
