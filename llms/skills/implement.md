...
Assign the resolved PR to that user:

```bash
gh pr edit <pr_number> --add-assignee <username>
```

Then resolve the associated issue:

- If the original input was an issue number, use that directly.
- If the original input was a PR number, extract the linked issue:
  ```bash
  gh pr view <pr_number> --json closingIssuesReferences \
    --jq '.closingIssuesReferences[].number'
  ```

If an associated issue is found, assign it to the same user:

```bash
gh issue edit <issue_number> --add-assignee <username>
```

If no associated issue is found, only the PR is assigned.
...
- Note project conventions: `uv` for builds, `pytest` + `hypothesis`
  for tests, the transpiler/dialect/expressions/generators architecture
  built on sqlglot.
